from trackers.ball_tracker import fine_fill_gap
...
def main():
    ...
    ball_tracker = BallTracker(model_path=load_or_export_model(device=0),fine_fill_service=ff)#use device=0 aka gpu
    # 检测网球，优先从缓存文件读取检测结果
    ball_detections, ball_bounce_frame = ball_tracker.detect_frames(video_frames,
                                                     read_from_stub=False,  #False True
                                                     stub_path="tracker_stubs/ball_detections.pkl"
                                                     )
    ball_detections=ff.fine_filling_gap(ball_detections)
